...
router.get('/', authenticate, authorize('developer', 'owner'), async (req, res) => {
  try {
    const [assignments] = await pool.query(`
      ...
      WHERE r.name = 'school_teacher' AND ua.school_id IS NOT NULL
...
// Get teachers list (for assignment dropdown)
...
// Get schools assigned to current teacher
...
router.post('/', authenticate, authorize('developer', 'owner'), async (req, res) => {
  try {
    ...
    const [result] = await pool.query(`
      INSERT INTO user_assignments (user_id, school_id, is_primary)
      VALUES (?, ?, false)
    `, [teacher_id, school_id]);
