...
// Get all trainer assignments (admin/trainer_head only)
router.get('/', authenticate, authorize('developer', 'owner', 'trainer_head'), async (req, res) => {
  try {
    const [assignments] = await pool.query(`
      SELECT ta.*, 
             u.first_name as trainer_first_name, u.last_name as trainer_last_name, u.email as trainer_email,
...
// Get trainers list (for assignment dropdown)
...
// Get schools assigned to current trainer
...
// Get centers assigned to current trainer
...
// Assign trainer to school/center (admin/trainer_head only)
router.post('/', authenticate, authorize('developer', 'owner', 'trainer_head'), async (req, res) => {
  try {
    ...
    const [result] = await pool.query(`
      ...
      VALUES (?, ?, ?, ?)
    `, [trainer_id, school_id || null, center_id || null, req.user.id]);
...
// Remove trainer assignment (admin/trainer_head only)
