  ...
  const loadAvailableEntities = async (userData) => {
    try {
      ...
      if (userData.role_name === 'trainer') {
        ...
        setAvailableCenters(centersRes.data);
        
        // Auto-select first available and set section based on what's available
        if (schoolsRes.data.length > 0 && !localStorage.getItem('selectedSchool')) {
          selectSchool(schoolsRes.data[0]);
        }
        if (centersRes.data.length > 0 && !localStorage.getItem('selectedCenter')) {
          selectCenter(centersRes.data[0]);
        }
        ...
        if (!localStorage.getItem('currentSection')) {
          ...
          } else if (centersRes.data.length > 0) {
