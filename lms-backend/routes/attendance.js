...
router.get('/school/:schoolId/date/:date', authenticate, async (req, res) => {
  try {
    ...
    let query = `SELECT a.*, s.first_name, s.last_name, c.name as class_name, te.subject, te.period_number FROM attendance a JOIN students s ON a.student_id = s.id LEFT JOIN classes c ON a.class_id = c.id LEFT JOIN timetable_entries te ON a.timetable_entry_id = te.id WHERE a.school_id = ? AND a.attendance_date = ? AND a.attendance_type = 'school'`;
...
router.get('/school/:schoolId/students/:date', authenticate, async (req, res) => {
  try {
    ...
    let query = `SELECT DISTINCT s.id, s.first_name, s.last_name, s.class_id, c.name as class_name, c.grade, c.section, te.id as timetable_entry_id, te.period_number, te.subject, te.start_time, te.end_time FROM students s JOIN classes c ON s.class_id = c.id JOIN timetables t ON t.class_id = c.id AND t.is_active = true JOIN timetable_entries te ON te.timetable_id = t.id AND te.day_of_week = ? WHERE s.school_id = ? AND s.student_type = 'school' AND s.is_active = true`;
    const params = [dayOfWeek, schoolId];
    if (classId) { query += ' AND s.class_id = ?'; params.push(classId); }
    query += ' ORDER BY c.name, te.period_number, s.first_name';
    const [students] = await pool.query(query, params);
    const [existingAttendance] = await pool.query(`SELECT student_id, timetable_entry_id, status FROM attendance WHERE school_id = ? AND attendance_date = ? AND attendance_type = 'school'`, [schoolId, date]);
    const attendanceMap = {};
    existingAttendance.forEach(a => { attendanceMap[`${a.student_id}-${a.timetable_entry_id}`] = a.status; });
    const studentsWithAttendance = students.map(s => ({ ...s, existing_status: attendanceMap[`${s.id}-${s.timetable_entry_id}`] || null }));
...
router.post('/school/mark', authenticate, authorize('developer', 'trainer', 'trainer_head'), async (req, res) => {
  try {
    ...
    for (const record of records) {
      const [existing] = await pool.query(`SELECT id FROM attendance WHERE student_id = ? AND attendance_date = ? AND timetable_entry_id = ? AND attendance_type = 'school'`, [record.student_id, attendance_date, record.timetable_entry_id]);
      if (existing.length > 0) {
        await pool.query(`UPDATE attendance SET status = ?, remarks = ?, marked_by = ? WHERE id = ?`, [record.status, record.remarks, req.user.id, existing[0].id]);
      } else {
        await pool.query(`INSERT INTO attendance (student_id, attendance_type, school_id, class_id, timetable_entry_id, period_number, attendance_date, status, remarks, marked_by) VALUES (?, 'school', ?, ?, ?, ?, ?, ?, ?, ?)`, [record.student_id, school_id, class_id, record.timetable_entry_id, record.period_number, attendance_date, record.status, record.remarks, req.user.id]);
      }
    }
...
router.get('/school/:schoolId/range', authenticate, async (req, res) => {
  try {
    ...
    let query = `SELECT a.id, a.student_id, a.attendance_date, a.status FROM attendance a WHERE a.school_id = ? AND a.attendance_type = 'school' AND a.attendance_date >= ? AND a.attendance_date <= ?`;
...
router.get('/center/:centerId', authenticate, async (req, res) => {
  try {
    ...
    let query = `SELECT a.*, s.first_name, s.last_name FROM attendance a JOIN students s ON a.student_id = s.id WHERE a.center_id = ? AND a.attendance_type = 'center'`;
    const params = [centerId];
    if (startDate) { query += ' AND a.attendance_date >= ?'; params.push(startDate); }
    if (endDate) { query += ' AND a.attendance_date <= ?'; params.push(endDate); }
...
router.post('/center/mark', authenticate, authorize('developer', 'trainer', 'trainer_head', 'registrar'), async (req, res) => {
  try {
    ...
    const [existing] = await pool.query(`SELECT id FROM attendance WHERE student_id = ? AND attendance_date = ? AND attendance_time = ? AND attendance_type = 'center'`, [student_id, attendance_date, attendance_time || null]);
    let result;
    if (existing.length > 0) {
      await pool.query(`UPDATE attendance SET status = ?, remarks = ?, marked_by = ? WHERE id = ?`, [status, remarks, req.user.id, existing[0].id]);
      result = { id: existing[0].id, updated: true };
    } else {
      const [insertResult] = await pool.query(`INSERT INTO attendance (student_id, attendance_type, center_id, attendance_date, attendance_time, status, remarks, marked_by) VALUES (?, 'center', ?, ?, ?, ?, ?, ?)`, [student_id, center_id, attendance_date, attendance_time, status, remarks, req.user.id]);
...
router.get('/summary/student/:studentId', authenticate, async (req, res) => {
  try {
    ...
    let query = `SELECT COUNT(*) as total_records, SUM(CASE WHEN status = 'present' THEN 1 ELSE 0 END) as present_count, SUM(CASE WHEN status = 'absent' THEN 1 ELSE 0 END) as absent_count, SUM(CASE WHEN status = 'late' THEN 1 ELSE 0 END) as late_count, SUM(CASE WHEN status = 'excused' THEN 1 ELSE 0 END) as excused_count FROM attendance WHERE student_id = ?`;
    const params = [studentId];
    if (startDate) { query += ' AND attendance_date >= ?'; params.push(startDate); }
